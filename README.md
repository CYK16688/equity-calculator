...
本项目以 [MIT License](LICENSE) 发布。第三方依赖仍受其各自许可证约束；当前运行时依赖 `@relation-graph/web-components` 使用 MIT License。

## 免责声明

本工具用于信息整理和估算，不保证输入数据的完整性或准确性，也不替代律师、会计师、审计师或投资顾问的专业意见。使用者应自行核验企业登记、股东名册、协议安排和融资条款等原始材料。
